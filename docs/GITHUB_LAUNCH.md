...
- Keep course-design reports on an explicit opt-in fastline so normal experiment reports stay stable.
...
- Stable path is focused on experiment reports; course-design support is opt-in
- Other report profiles are sample/regression assets until verified against real templates
- `.doc` conversion depends on local Word/WPS automation support
