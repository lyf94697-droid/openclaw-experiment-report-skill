...
- 自动生成正文以及默认 validation / layout-check 现在都会从 `profiles/experiment-report.json` 读取实验报告 profile；需要切换或覆盖时，可对 `build-report.ps1` / `build-report-from-url.ps1` / `build-report-from-feishu.ps1` 使用 `-ReportProfileName` 或 `-ReportProfilePath`
